...
if not video.isOpened():
    ...
    msgBox.setText("Impossible de trouver la caméra.")
    msgBox.setStandardButtons(QMessageBox.Ok)
    msgBox.exec_()
    while True:
        # Attendre indéfiniment jusqu'à ce que l'utilisateur ferme la fenêtre de message
...
with mp_hand.Hands(min_detection_confidence=0.5,
    ...
    while True:
        ...
        if not ret:
            msgBox = QMessageBox()
            msgBox.setIcon(QMessageBox.Critical)
            msgBox.setText("Problème avec la caméra.")
            msgBox.setStandardButtons(QMessageBox.Ok)
            msgBox.exec_()
            while True:
                # Attendre indéfiniment jusqu'à ce que l'utilisateur ferme la fenêtre de message
